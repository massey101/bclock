...
void show_time(int x, int y, vdatetime_t * datetime) {
    ...
    paint_SetHeight(&paint, 152);
    paint_Clear(&paint, UNCOLORED);
    paint_DrawStringAt(&paint, 6, 10, time_text, &Courier_New48, 1, COLORED);
    epd_SetFrameMemory(
...
void show_time_pointer(int epd_x, int epd_y) {
    ...
    switch(current_state) {
        case SET_DATE_HOUR1:         x = 6 + 6 + 29*0; y = 0; break;
        case SET_DATE_HOUR2:         x = 6 + 6 + 29*1; y = 0; break;
        case SET_DATE_MINUTE1:       x = 6 + 6 + 29*3; y = 0; break;
        case SET_DATE_MINUTE2:       x = 6 + 6 + 29*4; y = 0; break;
        default: x = 0; y = 0; break;
    }
...
void view_update(vdatetime_t * datetime, valarm_t * alarms) {
    ...
    if (
        ...
        show_dow(0x10, 0x38, datetime);
        if (current_state == SET_DATE_DOW) {
            show_dow_pointer(0x00, 0x40);
        }
    }
    ...
    if (
        ...
        show_date(0x10, 0x50, datetime);
        if (
            current_state >= SET_DATE_DAY1 &&
            current_state <= SET_DATE_YEAR2
        ) {
            show_date_pointer(0x10, 0x68);
        }
    }
